...
function loadModule (request, parent, callback, performCaching) {
  ...
  if (performCaching) {
    cachedModule = internalModuleCache[id] || parent.moduleCache[id];
...
function hotLoadModule (request, parent, callback) {
  var resolvedModule = resolveModulePath(request, parent),
      id = resolvedModule[0];

  return loadModule(request, parent, callback, natives[id] ? true : false);
}

function cachedLoadModule (request, parent, callback) {
  return loadModule(request, parent, callback, true);
};
...
function cat (id, callback) {
  if (id.match(/^http:\/\//)) {
    cachedLoadModule('http', process.mainModule, function (err, http) {
      if (err) {
...
Module.prototype._compile = function (content, filename) {
  ...
  function requireAsync (url, cb) {
    cachedLoadModule(url, self, cb);
  }

  function require (path) {
    return cachedLoadModule(path, self);
  }

  require.hot = function (path) {
    return hotLoadModule(path, self);
  };
